...
function testRecordKeepingNeverOffered() {
    var excluded = [
        ...
        // a data-use embargo date (BV-BRC restricted_until), in every spelling
        'x:Restricted Until', 'x:restricted_until', 'x:restricted-until', 'x:restrictedUntil',
        'x:RestrictedUntil', 'x:RESTRICTED_UNTIL', 'x:restricteduntil', 'BVBRC:restricted_until',
    ...
    var kept = [
        ...
        'x:Restricted',     // not the phrase "restricted until"
        'x:Unrestricted', 'x:Restriction',
        'x:Metadata',
...
// The DATA half of the joint contract: real phyloXML trees, one per rule,
// with every verdict, tier, rank and flag the classifier must produce, and
// what the viewer would open the tree with. The name fixture above cannot
// see any of this -- every divergence found on 2026-09-11/12 (sparse, the
// all-distinct numeric, applies_to="clade", the wide-repeat demotion) was in
// data-driven candidacy, and "163/163" only ever meant we agreed on spelling.
// Generated by test/fixtures/gen-vis-trees.js; regenerate deliberately.
function testJointContractTrees() {
    var dir = pth.join(__dirname, 'fixtures', 'vis-trees');
    var rows = fs.readFileSync(pth.join(__dirname, 'fixtures', 'vis-trees.tsv'), 'utf8')
        .split('\n').filter(function (l) { return l.length > 0 && l.charAt(0) !== '#'; })
    ...
    var cache = {};
    function candidatesOf(tree) {
        if (!cache[tree]) {
            var xml = fs.readFileSync(pth.join(dir, tree + '.xml'), 'utf8');
            var phy = px.parse(xml, {trim: true, normalize: true})[0];
            cache[tree] = forester.visualizationCandidates(phy);
        }
        return cache[tree];
    }
    var bad = [];
    ...
    rows.forEach(function (r) {
        var tree = r[0], ref = r[1];
        declared[tree] = declared[tree] || {};
        declared[tree][ref] = true;
        ...
        if (r[2] === 'refused') {
            if (idx >= 0) { bad.push(tree + ' ' + ref + ': offered, fixture says refused'); }
            return;
        }
        if (idx < 0) { bad.push(tree + ' ' + ref + ': refused, fixture says offered'); return; }
        var c = cands[idx];
        var got = [tierOf(c), idx, c.numeric ? 1 : 0, c.wide ? 1 : 0, c.nearUnique ? 1 : 0,
                   c.sparse ? 1 : 0, c.deprioritized ? 1 : 0, c.colorMode, c.shape ? 1 : 0].join('\t');
        var want = r.slice(3, 12).join('\t');
        if (got !== want) {
    ...
    Object.keys(declared).forEach(function (tree) {
        candidatesOf(tree).forEach(function (c) {
            if (!declared[tree][c.ref || c.id]) {
                bad.push(tree + ': offers undeclared ' + (c.ref || c.id));
            }
        });
    });
    if (bad.length) {
